...
import { UserInput, JsonRequest, UserParentInput, UserChildInput } from '../types';
...
export const CalculationContext = createContext<CalculationContextProps | undefined>(undefined);
...
export const CalculationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [jsonRequest, setJsonRequest] = useState<JsonRequest>({ data: { sumA: 0, multiplyB: 0 } });
  const { userInput } = useContext(InputContext)!;

  const calculationFunctions: Record<keyof UserParentInput, CalculationFunction> = {
    sumA: {
      name: 'sumA',
  ...
  useEffect(() => {
    ...
    const calculate = (calculationFunction: CalculationFunction | ChildCalculationFunction): number => {
      if ('isEntered' in calculationFunction && calculationFunction.isEntered) {
        ...
        const inputs = calculationFunction.inputKeys.map(key => userInput[key] || 0);
        return calculationFunction.calculationFunction(inputs);
      }
    };
  
    Object.entries(calculationFunctions).forEach(([key, calculationFunction]) => {
  ...
  return (
    <CalculationContext.Provider value={{ calculationResults, jsonRequest, calculationFunctions }}>
      {children}
